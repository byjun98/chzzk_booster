...
VAS 응답 보정 (adBreaks 비우기 + skipPreRollAd 강제)
  |
  | 막지 못한 광고(GFP fxview 오버레이 등)가 뜬 경우
  v
광고 순삭 (광고 영상만 끝으로 점프 + 10배속)
  |
  | 순삭으로도 남은 광고에 SKIP 버튼이 뜬 경우
  v
SKIP 버튼 자동 클릭
  |
  | 광고 차단 감지 팝업이 뜬 경우
...
VAS 응답 보정은 `adBreaks`를 비우는 것에 더해 치지직 API 응답의 `skipPreRollAd` 값을 `true`로 바꿔 플레이어가 프리롤을 건너뛰도록 유도합니다.

GFP `fxview` 오버레이 광고는 응답 보정으로 막기 어렵습니다. 이 경우 광고가 떠 있는 동안 짧은 간격으로 광고 영상만 골라 `currentTime`을 끝으로 보내고 `playbackRate`를 높여 순삭합니다. 광고 영상은 길이가 유한하고 짧다는 점(라이브는 무한/대용량 DVR)으로 구분해, 라이브 영상은 절대 조작하지 않습니다. 그래도 남는 광고는 스킵 버튼(`skip_area`, `txt_skip`)이 활성화되는 즉시 클릭하며, "광고 페이지 보기"(`link_more`)는 절대 클릭하지 않습니다.
